...
namespace Saturn.UI.Dialogs
{
    public class ModeEditorDialog : Dialog
    {
        ...
        public ModeEditorDialog(Mode? existingMode = null, OpenRouterClient? client = null, ToolRegistry? toolRegistry = null)
        ...
        {
            var toolDialog = new ToolSelectionDialog(selectedTools, toolRegistry);
        ...
        {
            var registry = this.toolRegistry ?? new ToolRegistry(null!); // Fallback for backward compatibility
            var allToolsCount = registry.GetAll().Count();
            if (selectedTools.Count == 0)
            {
                toolCountLabel.Text = $"No tools selected (0/{allToolsCount} available)";
            }
            else
            {
                toolCountLabel.Text = $"{selectedTools.Count}/{allToolsCount} tools selected";
            }
        }
